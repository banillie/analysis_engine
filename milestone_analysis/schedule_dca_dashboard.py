...
Programme for creating an aggregate portfolio schedule dashboard
...
scheduled confidence should remain on the dashboard. the others should be manually delete. The schedule narrative
provided for each project should be checked.

Instructions:
1) provide path to dashboard master
2) provide path and specify file name for output document
...
def placing_excel(dict_one, dict_two):
    ...
    for row_num in range(2, ws.max_row + 1):
        ...
        if project_name in dict_one:
            ...
            ws.cell(row=row_num, column=7).value = dict_one[project_name]['SRO Schedule Confidence']
            ws.cell(row=row_num, column=8).value = dict_one[project_name]['Milestone Commentary']

    for row_num in range(2, ws.max_row + 1):
        project_name = ws.cell(row=row_num, column=3).value
        if project_name in dict_two:
            try:
                ws.cell(row=row_num, column=5).value = dict_two[project_name]['SRO Schedule Confidence']
            except KeyError:
                pass
...
dash_keys = ['Total Forecast', 'SRO Schedule Confidence']

narrative = ['Milestone Commentary']

all_keys = dash_keys + narrative

'''key of interest for previous quarter'''
dash_keys_previous_quarter = ['SRO Schedule Confidence']
...
wb.save('C:\\Users\\Standalone\\general\\masters folder\\portfolio_milestones\\q2_1920_schedule_dashboard.xlsx')