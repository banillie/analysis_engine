...
Place into the input folder the an excel version of the dm but with column A
information only. Title this document commission_master_test (or what
ever you want to call its as long as its the same name as first file path 
below.)
Run the programme
A master will be produced and placed in the output folder titled
commission_master_test_populated (or what ever you title it in second
file path. This doc can be used in Datamaps. Note. Copy all project names
in row 1 so they are in row 2 also"""
master_dm = load_workbook(root_path/'input/commission_master_test.xlsx')

run = create_master(master_dm, q1_2021.projects, q1_2021, q1_2021)

run.save(root_path/'output/commission_master_test_populated.xlsx')


# dm_master_dict = project_data_from_master('/home/will/Documents/datamaps/dm_excel_master_2.6.xlsx', 4, 2020)
#
# cell_key_list = []
# for v in dm_master_dict.data['cell_reference'].values():
