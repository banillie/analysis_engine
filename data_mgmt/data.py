...
VFM_LIST = ['NPV for all projects and NPV for programmes if available',
    ...
    def get_count(self) -> None:
        ...
        for quarter in self.vfm_dictionary.keys():
            ...
            for y, project in enumerate(list(self.vfm_dictionary[quarter].keys())):
                cat_list.append(self.vfm_dictionary[quarter][project]["VfM Category single entry"])

            output_dict[quarter] = Counter(cat_list)

        self.vfm_cat_count = output_dict

        output_dict = {}
        for quarter in self.vfm_cat_count.keys():
            output_list = []
            for cat in self.vfm_cat_count[quarter].keys():
                pvc_count = 0
                total_count = 0
                for y, project in enumerate(list(self.vfm_dictionary[quarter].keys())):
                    try:
                        total_count += self.vfm_dictionary[quarter][project]["Present Value Cost (PVC)"]
                        proj_cat = self.vfm_dictionary[quarter][project]["VfM Category single entry"]
                        if proj_cat == cat:
                            pvc_count += self.vfm_dictionary[quarter][project]["Present Value Cost (PVC)"]
                    except TypeError:
                        print(project + " PVC data needs checking")
                        pass
                output_list.append((cat, pvc_count))
            output_list.append(('Total', total_count))
            output_dict[quarter] = dict(output_list)

        self.vfm_cat_pvc = output_dict


        #                 total += 1
        #                 try:
        #                     cost_total += self.vfm_dictionary[quarter][vfm_type][
        #                         project
        #                     ]["Costs"]
        #                 except TypeError:  # TODO error message handling
        #                     print(
        ...
        #                     pass
        #                 if (
        #                     self.vfm_dictionary[quarter][vfm_type][project]["DCA"]
        #                     == cat
        #                 ):
        #                     count += 1
        #                     try:
        #                         cost += self.dca_dictionary[quarter][vfm_type][project][
        #                             "Costs"
        #                         ]
        #                     except TypeError:  # error message above doesn't need repeating
        #                         pass
        #             cat_count.append((cat, (count, cost, cost / cost_total)))
        #             total_count.append(
        #                 ("Total", (total, cost_total, cost_total / cost_total))
        #             )
        #
        #         vfm_dict[vfm_type] = dict(cat_count + total_count)
        #     output_dict[quarter] = vfm_dict
        #
        # self.dca_count = output_dict
...
def vfm_into_excel(vfm_data: VfMData, quarter: List[str] or str) -> workbook:
    ...
    for q in quarter:
        ...
        for i, project_name in enumerate(list(vfm_data.vfm_dictionary[q].keys())):
            ...
            for x, key in enumerate(list(vfm_data.vfm_dictionary[q][project_name].keys())):
                ws.cell(row=2, column=3 + x).value = key
                ws.cell(row=start_row + i, column=3 + x).value = vfm_data.vfm_dictionary[q][project_name][key]
        #
        # for i, vfm_type in enumerate(list(dca_data.dca_count[q].keys())):
        #     ws.cell(row=start_row + i, column=2).value = vfm_type
        #     ws.cell(row=start_row + i, column=3).value = "Count"
        #     ws.cell(row=start_row + i, column=4).value = "Costs"
        #     ws.cell(row=start_row + i, column=5).value = "Proportion costs"
        #     for x, colour in enumerate(list(dca_data.dca_count[q][vfm_type].keys())):
        #         ws.cell(row=start_row + i + x + 1, column=2).value = colour
        #         ws.cell(row=start_row + i + x + 1, column=3).value = (
        #             dca_data.dca_count[q][vfm_type][colour]
        #         )[0]
        #         ws.cell(row=start_row + i + x + 1, column=4).value = (
        #             dca_data.dca_count[q][vfm_type][colour]
        #         )[1]
