...
VFM_CAT = ["Poor", "Low", "Medium", "High", "Very High",
           "Very High and Financially Positive",
           "Economically Positive", "Total"]
...
def vfm_into_excel(vfm_data: VfMData, quarter: List[str] or str) -> workbook:
    ...
    for x, q in enumerate(quarter):
        ...
        for i, cat in enumerate(VFM_CAT):
            ...
            try:
                ws.cell(row=start_row + i, column=3 + x).value = vfm_data.vfm_cat_pvc[q][cat]
                ws.cell(row=start_row + i + 12, column=3 + x).value = vfm_data.vfm_cat_count[q][cat]
            except KeyError:
                pass
