...
from analysis.data import q2_1920, q1_1920, financial_analysis_masters_list, q2_1920, baseline_bcs, \
    bc_index, red_text, income_list, cost_list, wlc_key, latest_quarter_project_names
    ...
    for row_num in range(2, ws.max_row + 1):
        ...
        if project_name in latest_quarter_project_names:
            ...
            try:
                ...
                ws.cell(row=row_num, column=5).value = wlc_now - wlc_lst_quarter
            except KeyError:
                ws.cell(row=row_num, column=5).value = 'n/a'
            '''WLC variance against baseline quarter'''
            wlc_baseline = financial_analysis_masters_list[bc_index[project_name][2]].data[project_name]['Total Forecast']
            ws.cell(row=row_num, column=6).value = wlc_now - wlc_baseline
