"""
cml for analysis engine.
currently working on one example - vfm analysis

Arguments
vfm: runs analysis
-group: specifies a particular dft group of projects. str
-stage: specifies a group of projects at a particular stage. str
-quarter: specifies the quarter(s) for analysis. List[str]

-stage and -group cannot be entered at same time.

Once above established:
- build other cml arguments for other analysis engine outputs e.g. milestones.
- explore possibility of there being a way to 'initiate' analysis engine so
master data is stored in memory and arguments run directly from it. rather
than having to convert excel ws into python dict each time.
- have cml so that it is analysis_engine, rather than main.py

"""

import argparse
# from vfm.vfm_analysis import compile_vfm_analysis


from data_mgmt.data import (
    get_master_data,
    Master,
    get_project_information, VfMData, root_path, vfm_into_excel
)


def main():
    parser = argparse.ArgumentParser(description='value for money analysis')
    parser.add_argument('-vfm',
                        metavar='run',
                        default='run',
                        action='store',
                        help='runs vfm analysis')
    parser.add_argument('-group',
                        metavar='run',
                        action='store',
                        help='returns a specified group')
    args = vars(parser.parse_args())
    if args["vfm"] == "run":
        m = Master(get_master_data(), get_project_information())
        current_quarter = str(m.master_data[0].quarter)
        last_quarter = str(m.master_data[1].quarter)
        quarter_list = [current_quarter, last_quarter]
        vfm = VfMData(m)
        vfm.get_dictionary()
        vfm.get_count()
        wb = vfm_into_excel(vfm, quarter_list)
        wb.save(root_path / "output/vfm.xlsx")
        print("compiling vfm analysis")


if __name__ == "__main__":
    main()

