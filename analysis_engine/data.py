...
def get_group(master, tp: str, class_kwargs, group_indx=None) -> List[str]:
    if "baseline" in class_kwargs:
...
def financial_dashboard(master, wb: Workbook, **kwargs) -> Workbook:
    ws = wb['Finance']
...
def ipdc_dashboard(master, wb: Workbook, kwargs) -> Workbook:
...
def cost_stackplot_graph(
        sp_dict: Dict[str, float], master, **kwargs
) -> plt.figure:
