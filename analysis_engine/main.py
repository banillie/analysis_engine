...
from data import (
    ...
    run_p_reports, RiskData, risks_into_excel
)
...
def vfm(args):
    ...
    vfm_m = VfMData(m)  # why does this need to come first and not as else statement below? 
    if args["quarters"]:
...
def risks(args):
    ...
    m = Master(get_master_data(), get_project_information())
    risk_m = RiskData(m)  # why does this need to come first and not as else statement below?
    if args["quarters"]:
...
def main():
    ...
    parser_risks = subparsers.add_parser('risks',
                                         help='risk analysis')
    ...
    parser_risks.add_argument('--stage',
                            type=str,
                            metavar='',
                            action='store',
                            nargs='+',
                            ...
                            help='Returns analysis for those projects at the specified planning stage(s). Must be one '
                                 'or combination of "FBC", "OBC", "SOBC", "pre-SOBC".')
    parser_risks.add_argument('--group',
                            type=str,
                            metavar='',
                            action='store',
                            nargs='+',
                            ...
                            help='Returns analysis for those projects in the specified DfT Group. Must be one or '
                                 'combination of "HSMRPG", "AMIS", "Rail", "RDM"')
    parser_risks.add_argument('--quarters',
                            type=str,
                            metavar='',
                            action='store',
                            nargs='+',
